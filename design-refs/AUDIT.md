...
| **Real iOS behaviour** | Everything about the status bar, the home-screen shortcut, `100dvh` and the sticky `:hover` fills is reasoned from the platform's documented behaviour and verified only in Chromium. **Unverified on device.** This is the largest single block of unverified surface in the project, and the only one a phone closes in an afternoon. `DEVICE-CHECK.md` is the checklist for that session, written for somebody using the app with their own money rather than for somebody holding this file. |
