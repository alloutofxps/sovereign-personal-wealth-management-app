 ...
 */
function markSvg(size, inset, bleed) {
  ...
  ${background}
  <rect x="${pad}" y="${top}" width="${width}" height="${barHeight}" rx="${radius}" fill="${LIQUID}"/>
  <rect x="${pad}" y="${top + barHeight + gap / 2 - size * 0.004}" width="${width}" height="${Math.max(1, size * 0.008)}" fill="${RULE}"/>
  <rect x="${pad}" y="${top + barHeight + gap}" width="${width}" height="${barHeight}" rx="${radius}" fill="${ZINC}"/>
</svg>`;
}
...
for (const { file, size, inset, bleed } of targets) {
  const svg = markSvg(size, inset, bleed);
